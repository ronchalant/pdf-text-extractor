package com.ronchalant.pdftextcleaner.util;
 ...
 * 'a'.
 */
public final class UnaccentUtil {


    public static final String unaccent(final String s) {
        return unaccent(s, true);
    }

    public static final String unaccent(final String s, final boolean defaultPassThrough) {
        if (s == null) return null;
        final StringBuffer sb = new StringBuffer();
        unaccent(s, sb, defaultPassThrough);
        return sb.toString();
    }

    public static final void unaccent(final String s, final StringBuffer out) {
        unaccent(s, out, true);
    }

    public static final void unaccent(final String s, final StringBuffer out, final boolean defaultPassThrough) {
        if (s == null) {
            return;
        }

        for (char c : s.toCharArray()) {
            unaccent(c, out, defaultPassThrough);
        }
    }
     ...
     */
    public static final String unaccent(final char c)
    {
        return unaccent(c, true);
    }

    public static final String unaccent(final char c, final boolean defaultPassThrough)
    {
     ...
     * @return converted value
     */
    public static final void unaccent(final char c, final StringBuffer out, final boolean defaultPassThrough)
    {
        final char[] output = new char[3];
        int outputPos = 0;
        ...
        if (outputPos > 0) {
            out.append(output, 0, outputPos);
        }
    }


    protected static char[] unaccentChar(char c) {
...
//			break;
        ...
        return ArrayUtils.subarray(output, 0, outputPos);
    }

    public static void main(String[] args) throws Exception {

        System.out.println("èé".toUpperCase());
//		StringBuffer from = new StringBuffer();
//		StringBuffer to = new StringBuffer();
//		for (int i = 0x0000; i <= /*0xffff*/535; i++) {
//		// for (char c : test.toCharArray()) {
//			char cIn = (char)i;
//			char[] unaccented = AccentUtil.unaccentChar(cIn);
//			if (unaccented.length == 1 && unaccented[0] == cIn) {
//				continue;
//			}
//
//			char cOut = unaccented[0];
//
//			if ((cOut >= 'a' && cOut <= 'z') || (cOut >= 'A' && cOut <= 'Z')) {
//				System.out.println("[" + i + "] Converted: " + cIn + " to " + cOut);
//				from.append(cIn);
//				to.append(cOut);
//			}
//		}
//
//
//		String oracleSux = "ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿAaAaAaCcCcCcCcDdÐdEeEeEeEeEeGgGgGgGgHhHhIiIiIiIiIi¿¿JjKk¿LlLlLl¿¿LlNnNnNn¿¿¿OoOoOoŒœRrRrRrSsSsSsŠšTtTtTtUuUuUuUuUuUuWwYyŸZzZzŽž¿b¿¿¿¿¿¿Ð¿¿¿¿¿¿ƒƒ¿¿¿I¿¿l¿¿¿OOo¿¿t¿¿TUu¿¿¿¿z¿¿¿¿¿¿¿¿¿¿AaIiOoUuUuUuUuUu¿Aa¿¿¿¿GgGgKkOoOoj¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿¿";
//		System.out.println((int)oracleSux.charAt(oracleSux.length()-1));
//		for (int i = oracleSux.length()-1; i >= 0 ; i--) {
//			if (191 == (int)oracleSux.charAt(i)) {
...
//		}
//
//		System.out.println(oracleSux.length() == from.length());
//
//		System.out.println("translate(?, '" + from.toString() + "', '" + to.toString() + "')");
//
//
    }
}
